...
def generate_mst_server(log_returns_df, mst_algorithm='kruskal', distance_matrix_type='angular',
    ...
    mat = create_input_matrix(log_returns_df, distance_matrix_type)
    mst = MST(mat, "distance", mst_algorithm)
    if colours:
        mst.set_node_groups(colours)
    if sizes:
        mst.set_node_size(sizes)
    return DashGraph(mst, app_display='jupyter' if jupyter else 'default').get_server()
...
def create_input_matrix(log_returns_df, distance_matrix_type):
    ...
    return get_distance_matrix(corr, distance_matrix_type)
...
def generate_almst_server(log_returns_df, distance_matrix_type='angular',
    ...
    mat = create_input_matrix(log_returns_df, distance_matrix_type)
    almst = ALMST(mat, "distance")
    if colours:
        almst.set_node_groups(colours)
    if sizes:
        almst.set_node_size(sizes)
    return DashGraph(almst, app_display='jupyter' if jupyter else 'default').get_server()
...
def generate_mst_almst_comparison(log_returns_df, distance_matrix_type='angular', jupyter=False):
    ...
    mat = create_input_matrix(log_returns_df, distance_matrix_type)
    mst = MST(mat, "distance")
    almst = ALMST(mat, "distance")
    return DualDashGraph(mst, almst, app_display='jupyter' if jupyter else 'default').get_server()
...
def generate_pmfg_server(log_returns_df, input_type='distance',
    ...
    mat = log_returns_df.corr()
    if input_type == 'distance':
        mat = get_distance_matrix(mat, 'angular')
    pmfg = PMFG(mat, input_type)
    if colours:
        pmfg.set_node_groups(colours)
    if sizes:
        pmfg.set_node_size(sizes)
    return PMFGDash(pmfg, app_display='jupyter' if jupyter else 'default').get_server()
...
def generate_central_peripheral_ranking(nx_graph):
    ...
    clo = nx.closeness_centrality(nx_graph)
    bet = nx.betweenness_centrality(nx_graph)
    score = {n: deg[n] + clo[n] + bet[n] for n in nx_graph.nodes()}
