...
class FractionalDifferentiation:
    ...
    def frac_diff(series, diff_amt, thresh=0.01):
        ...
        w = FractionalDifferentiation.get_weights(diff_amt, series.shape[0])
        w = w[np.abs(w) > thresh]
        width = len(w)
    ...
    def frac_diff_ffd(series, diff_amt, thresh=1e-5):
        ...
        * https://en.wikipedia.org/wiki/Fractional_calculus
        ...
        w = FractionalDifferentiation.get_weights_ffd(diff_amt, thresh, series.shape[0])
        width = len(w)
        df = pd.DataFrame(index=series.index)
        for col in series.columns:
            vals = series[col].values
...
def frac_diff_ffd(series, diff_amt, thresh=1e-5):
    ...
    * https://en.wikipedia.org/wiki/Fractional_calculus
    ...
    return FractionalDifferentiation.frac_diff_ffd(series if isinstance(series, pd.DataFrame) else series.to_frame(),
                                                   diff_amt, thresh)
...
def plot_min_ffd(series):
    ...
    * https://www.wiley.com/en-us/Advances+in+Financial+Machine+Learning-p-9781119482086
    ...
    for d in np.linspace(0, 1, 11):
        df = frac_diff_ffd(series[['close']], d)
        adf = adfuller(df['close'].dropna(), maxlag=1, regression='c', autolag=None)[0]
        corr = np.corrcoef(series['close'].iloc[-len(df):].values, df['close'].values)[0, 1]
        out.append((d, adf, corr))
    out = pd.DataFrame(out, columns=['d', 'adf', 'corr']).set_index('d')
    ax = out['corr'].plot()
    ax2 = ax.twinx()
    out['adf'].plot(ax=ax2, color='r')
    return ax
