...
def gpr_distance(x: np.array, y: np.array, theta: float) -> float:
    ...
    dep = np.sqrt(0.5 * (1 - corr))
    dist = np.abs(np.mean(x) - np.mean(y)) / (np.std(x) + np.std(y) + 1e-12)
    return theta * dep + (1 - theta) * dist
...
def gnpr_distance(x: np.array, y: np.array, theta: float, n_bins: int = 50) -> float:
    ...
    x = np.asarray(x)
    y = np.asarray(y)
    dep = np.sqrt(0.5 * (1 - spearmans_rho(x, y)))
    x_sort = np.sort(x)
    y_sort = np.sort(y)
    dist = np.mean(np.abs(x_sort - y_sort)) / (np.std(np.concatenate([x, y])) + 1e-12)
    return theta * dep + (1 - theta) * dist
