...
def kullback_leibler_distance(corr_a, corr_b):
    ...
    n = a.shape[0]
    det_ratio = np.linalg.det(b) / np.linalg.det(a)
    return 0.5 * (np.log(det_ratio) + np.trace(np.linalg.solve(b, a)) - n)
...
def norm_distance(matrix_a, matrix_b, r_val=2):
    ...
    """
    return np.linalg.norm(np.asarray(matrix_a) - np.asarray(matrix_b), ord=r_val)
