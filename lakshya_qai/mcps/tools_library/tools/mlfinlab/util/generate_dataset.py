...
def get_classification_data(n_features=100, n_informative=25, n_redundant=25, n_samples=10000, random_state=0, sigma=.0):
    ...
    if n_redundant > 0 and sigma > 0:
        x[:, n_informative:n_informative + n_redundant] += np.random.normal(
            0, sigma, size=(n_samples, n_redundant))
    cols = (
        [f"I{i}" for i in range(n_informative)] +
        [f"R{i}" for i in range(n_redundant)] +
        [f"N{i}" for i in range(n_features - n_informative - n_redundant)]
    )
