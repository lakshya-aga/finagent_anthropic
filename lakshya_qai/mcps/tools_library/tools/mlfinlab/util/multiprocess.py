...
def lin_parts(num_atoms, num_threads):
    ...
    """
    num_threads = min(int(num_threads), int(num_atoms))
    if num_threads <= 1:
        return np.array([0, int(num_atoms)])
    parts = np.linspace(0, num_atoms, num_threads + 1)
    parts = np.round(parts).astype(int)
    parts[-1] = int(num_atoms)
    return parts
...
def nested_parts(num_atoms, num_threads, upper_triangle=False):
    ...
    if num_threads <= 1:
        return np.array([0, int(num_atoms)])
    total = num_atoms * (num_atoms + 1) / 2.0
    parts = []
    for i in range(num_threads + 1):
        frac = i / num_threads
        target = frac * total
        p = int((np.sqrt(1.0 + 8.0 * target) - 1.0) / 2.0)
        parts.append(p)
    parts = np.array(parts, dtype=int)
    parts[-1] = int(num_atoms)
    if upper_triangle:
        parts = np.array(sorted({int(num_atoms - p) for p in parts}))
    return parts
...
def mp_pandas_obj(func, pd_obj, num_threads=24, mp_batches=1, lin_mols=True, verbose=True, **kargs):
    ...
    if len(atoms) == 0:
        return pd.DataFrame()
    num_threads = min(int(num_threads), len(atoms))
    parts = lin_parts(len(atoms), num_threads * int(mp_batches)) if lin_mols else nested_parts(len(atoms), num_threads * int(mp_batches))
    jobs = []
    for i in range(1, len(parts)):
        job = dict(kargs)
        job["func"] = func
        job[pd_obj[0]] = atoms[parts[i - 1]:parts[i]]
        jobs.append(job)
    if num_threads == 1:
        out = process_jobs_(jobs)
    else:
        out = process_jobs(jobs, task=getattr(func, "__name__", "task"), num_threads=num_threads, verbose=verbose)
    if len(out) == 0:
        return pd.DataFrame()
...
def expand_call(kargs):
    ...
    """
    func = kargs["func"]
    args = {k: v for k, v in kargs.items() if k != "func"}
    return func(**args)
...
def report_progress(job_num, num_jobs, time0, task):
    ...
    :return: (None)
    """
    elapsed = (time.time() - time0) / 60.0
    msg = f"{job_num}/{num_jobs} {task} done after {elapsed:.2f} minutes."
    if job_num < num_jobs:
        remaining = elapsed * (num_jobs / job_num - 1.0)
        msg += f" {remaining:.2f} minutes remaining."
    if job_num == num_jobs:
        msg += " done."
...
def process_jobs(jobs, task=None, num_threads=24, verbose=True):
    ...
    time0 = time.time()
    out = []
    pool = mp.Pool(processes=int(num_threads))
    async_res = [pool.apply_async(expand_call, args=(job,)) for job in jobs]
    pool.close()
    for i, res in enumerate(async_res):
        out.append(res.get())
        if verbose:
            report_progress(i + 1, len(jobs), time0, task or "task")
    pool.join()
    return out
