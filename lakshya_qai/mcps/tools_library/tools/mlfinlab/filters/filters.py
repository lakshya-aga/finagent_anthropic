...
def cusum_filter(raw_time_series, threshold, time_stamps=True):
    ...
    for t, x in diff.items():
        ...
        if s_pos > thr:
            ...
            s_neg = 0.0
            events.append(t)
    return pd.DatetimeIndex(events) if time_stamps else events
...
def z_score_filter(raw_time_series, mean_window, std_window, z_score=3, time_stamps=True):
    ...
    """
    ts = raw_time_series.dropna()
    mean = ts.rolling(mean_window).mean()
    std = ts.rolling(std_window).std()
    z = (ts - mean) / std
    events = z[abs(z) > z_score].index
    return pd.DatetimeIndex(events) if time_stamps else list(events)
