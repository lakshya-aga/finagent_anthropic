...
def trend_scanning_labels(price_series: pd.Series, t_events: list = None, observation_window: int = 20,
    ...
    out = []
    for t in t_events:
        idx = price_series.index.get_loc(t)
        if look_forward:
            end = min(idx + observation_window, len(price_series) - 1)
            ranges = range(idx + min_sample_length, end + 1, step)
            best_t = 0.0
            best_t1 = t
            for j in ranges:
                y = price_series.iloc[idx:j + 1]
                X = pd.DataFrame({"trend": np.arange(len(y)) + 1})
                betas, var = get_betas(X, y.to_frame("y"))
                tval = betas[0] / np.sqrt(var[0]) if var[0] != 0 else 0.0
                if abs(tval) > abs(best_t):
                    best_t = tval
                    best_t1 = y.index[-1]
        else:
            start = max(idx - observation_window, 0)
            ranges = range(start, idx - min_sample_length + 1, step)
            best_t = 0.0
            best_t1 = t
            for j in ranges:
                y = price_series.iloc[j:idx + 1]
                X = pd.DataFrame({"trend": np.arange(len(y)) + 1})
                betas, var = get_betas(X, y.to_frame("y"))
                tval = betas[0] / np.sqrt(var[0]) if var[0] != 0 else 0.0
                if abs(tval) > abs(best_t):
                    best_t = tval
                    best_t1 = y.index[0]
        ret = price_series.loc[best_t1] / price_series.loc[t] - 1.0
        out.append([t, best_t1, best_t, ret, np.sign(best_t)])
    return pd.DataFrame(out, columns=["t0", "t1", "tvalue", "ret", "bin"]).set_index("t0")
