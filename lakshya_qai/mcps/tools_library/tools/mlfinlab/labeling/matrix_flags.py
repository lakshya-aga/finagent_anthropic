...
class MatrixFlagLabels:
    ...
    def __init__(self, prices, window, template_name=None):
        ...
        self.prices = prices
        self.window = window
        self.template = self._init_template(template_name) if template_name else pd.DataFrame(np.ones((10, 10)))
    ...
    def _init_template(self, name):
        ...
        if name is None:
            return pd.DataFrame(np.ones((10, 10)))
        if "bear" in name:
            return pd.DataFrame(-np.tril(np.ones((10, 10))))
        return pd.DataFrame(np.triu(np.ones((10, 10))))
    ...
    def _transform_data(self, row_num, window=30):
        ...
        data = self.prices.iloc[row_num - window + 1:row_num + 1]
        bins = np.array_split(data.values, 10)
        mat = np.zeros((10, 10))
        for col, bucket in enumerate(bins):
            if len(bucket) == 0:
                continue
            deciles = pd.qcut(data, 10, labels=False, duplicates='drop')
            bucket_idx = deciles.iloc[col * len(bucket):(col + 1) * len(bucket)]
            counts = bucket_idx.value_counts().reindex(range(10), fill_value=0).values
            mat[:, col] = counts / counts.sum() if counts.sum() > 0 else 0
    ...
    def _apply_template_to_matrix(self, matrix, template):
        ...
        return float((matrix.values * template.values).sum())
    ...
    def apply_labeling_matrix(self, threshold=None):
        ...
        scores = {}
        for i in range(self.window - 1, len(self.prices)):
            mat = self._transform_data(i, self.window)
            scores[self.prices.index[i]] = self._apply_template_to_matrix(mat, self.template)
