...
class EMAImbalanceBars(BaseImbalanceBars):
    ...
    def __init__(self, metric: str, num_prev_bars: int,
        ...
        """
        super().__init__(metric, batch_size, expected_imbalance_window, exp_num_ticks_init, analyse_thresholds)
        self.num_prev_bars = num_prev_bars
        self.exp_num_ticks_constraints = exp_num_ticks_constraints or [1, np.inf]

    def _get_exp_num_ticks(self):
        """
        Calculate the expected number of ticks per bar using an exponentially weighted moving average.

        Uses the EWMA of the historical number of ticks in previously formed bars to
        estimate the expected number of ticks for the next bar. The result is clamped
        to stay within the configured constraints defined by `exp_num_ticks_constraints`.

        If no bars have been formed yet, the method returns early without updating.
        """
        if len(self.num_ticks_bar) == 0:
            return
        window = min(self.num_prev_bars, len(self.num_ticks_bar))
        exp_ticks = float(ewma(np.array(self.num_ticks_bar, dtype=float), window)[-1])
        low, high = self.exp_num_ticks_constraints
        self.exp_num_ticks = max(low, min(high, exp_ticks))
...
def get_ema_dollar_imbalance_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], num_prev_bars: int = 3,
    ...
    """
    bars = EMAImbalanceBars("dollar", num_prev_bars, expected_imbalance_window, exp_num_ticks_init,
                            exp_num_ticks_constraints, batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
        return df, thresh
    return df
...
def get_ema_volume_imbalance_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], num_prev_bars: int = 3,
    ...
    """
    bars = EMAImbalanceBars("volume", num_prev_bars, expected_imbalance_window, exp_num_ticks_init,
                            exp_num_ticks_constraints, batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
...
def get_ema_tick_imbalance_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], num_prev_bars: int = 3,
    ...
    """
    bars = EMAImbalanceBars("tick", num_prev_bars, expected_imbalance_window, exp_num_ticks_init,
                            exp_num_ticks_constraints, batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
...
def get_const_dollar_imbalance_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], expected_imbalance_window: int = 10000,
    ...
    :return: (pd.DataFrame) DataFrame of dollar imbalance bars and DataFrame of thresholds, if to_csv=True returns None
    """
    bars = ConstImbalanceBars("dollar", expected_imbalance_window, exp_num_ticks_init, batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
        return df, thresh
...
def get_const_volume_imbalance_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], expected_imbalance_window: int = 10000,
    ...
    :return: (pd.DataFrame) DataFrame of volume imbalance bars and DataFrame of thresholds, if to_csv=True returns None
    """
    bars = ConstImbalanceBars("volume", expected_imbalance_window, exp_num_ticks_init, batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
        return df, thresh
...
def get_const_tick_imbalance_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], expected_imbalance_window: int = 10000,
    ...
    :return: (pd.DataFrame) DataFrame of tick imbalance bars and DataFrame of thresholds, if to_csv=True returns None
    """
    bars = ConstImbalanceBars("tick", expected_imbalance_window, exp_num_ticks_init, batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
        return df, thresh
