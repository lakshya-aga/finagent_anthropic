...
class EMARunBars(BaseRunBars):
    ...
    def __init__(self, metric: str, num_prev_bars: int,
        ...
        """
        super().__init__(metric, batch_size, num_prev_bars, expected_imbalance_window, exp_num_ticks_init, analyse_thresholds)
        self.exp_num_ticks_constraints = exp_num_ticks_constraints or [1, np.inf]

    def _get_exp_num_ticks(self):
        """
        Compute the expected number of ticks for the next bar using an exponentially weighted moving average.

        Uses the EWMA of the historical bar tick counts, constrained to lie within
        ``exp_num_ticks_constraints``. If no bars have been formed yet, returns early
        without updating the expectation.

        The window size for the EWMA is the minimum of ``num_prev_bars`` and the
...
def get_ema_dollar_run_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], num_prev_bars: int = 3,
    ...
    """
    bars = EMARunBars("dollar", num_prev_bars, expected_imbalance_window, exp_num_ticks_init,
...
def get_ema_volume_run_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], num_prev_bars: int = 3,
    ...
    """
    bars = EMARunBars("volume", num_prev_bars, expected_imbalance_window, exp_num_ticks_init,
                      exp_num_ticks_constraints, batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
...
def get_ema_tick_run_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], num_prev_bars: int = 3,
    ...
    """
    bars = EMARunBars("tick", num_prev_bars, expected_imbalance_window, exp_num_ticks_init,
                      exp_num_ticks_constraints, batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
...
def get_const_dollar_run_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], num_prev_bars: int,
    ...
    """
    bars = ConstRunBars("dollar", num_prev_bars, expected_imbalance_window, exp_num_ticks_init,
                        batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
        return df, thresh
...
def get_const_volume_run_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], num_prev_bars: int,
    ...
    :return: (pd.DataFrame) DataFrame of volume bars and DataFrame of thresholds
    """
    bars = ConstRunBars("volume", num_prev_bars, expected_imbalance_window, exp_num_ticks_init,
                        batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
...
def get_const_tick_run_bars(file_path_or_df: Union[str, Iterable[str], pd.DataFrame], num_prev_bars: int,
    ...
    :return: (pd.DataFrame) DataFrame of tick bars and DataFrame of thresholds
    """
    bars = ConstRunBars("tick", num_prev_bars, expected_imbalance_window, exp_num_ticks_init,
                        batch_size, analyse_thresholds)
    df = bars.batch_run(file_path_or_df, verbose=verbose, to_csv=to_csv, output_path=output_path)
    if analyse_thresholds and not to_csv:
        thresh = pd.DataFrame(bars.thresholds, columns=["date_time", "theta", "exp_num_ticks", "exp_imbalance"])
